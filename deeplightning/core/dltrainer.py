...
from deeplightning.utils.python_utils import flatten_dataclass, public_attributes
...
class DeepLightningTrainer(Trainer):
    ...
    def init_logger(self, cfg: DeepLightningConfig) -> None:
        ...
        if cfg.logger.provider == "wandb":
            ...
            logger.experiment.config.update(flatten_dataclass(cfg))

            # Intialize step label for each metrics
            metric_names = []
            phases = public_attributes(cfg.task.metrics)
            for x in phases:
                phase_metrics = getattr(cfg.task.metrics, x)
                for y in phase_metrics:
                    metric_names.append(f"{x}_{y}")
    ...
    def init_callbacks(self, cfg: DeepLightningConfig, artifact_path: str) -> list[Callback]:
        ...
        filename_str = (
            f"{{epoch}}-{{step}}-{{val_loss:.4f}}-"
            f"{{{cfg.stages.train.ckpt_monitor_metric}:.4f}}"
        )
        ...
        checkpoint = ModelCheckpoint(
            ...
            mode = cfg.stages.train.ckpt_monitor_mode,
